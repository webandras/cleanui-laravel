<div x-data="{
    ...
    <x-global::form-modal trigger="isModalOpen" title="{{ __('Are you sure you want to delete?') }}"
                        id="{{ $modalId }}">
        <form wire:submit.prevent="deleteOrganizer">
            <h2 class="h3">{{ $name }}</h2>
            <hr class="divider">

            <input wire:model.defer="organizerId"
                   disabled
                   type="number"
                   class="hidden"
                   name="organizerId"
            ...
            <div class="actions">
                <button type="submit" class="danger">
                    <span wire:loading wire:target="deleteOrganizer" class="animate-spin">&#9696;</span>
                    <span wire:loading.remove wire:target="deleteOrganizer">{{ __('Delete') }}</span>
                </button>
